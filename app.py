...
def load_components():
    ...
    def generate(state):
        ...
        prompt = PromptTemplate.from_template("Answer the user's question based only on the following context:\n\nContext:\n{context}\n\nQuestion:\n{question}")
        def format_docs(docs): return "\\n\\n".join(doc.page_content for doc in docs)
        
        rag_chain = prompt | generator_llm | StrOutputParser()
        
        # Format the context and then invoke the chain
...
@app.get("/")
def read_root(): return {"status": "Cogni-Compliance API is running"}

@app.post("/ask", response_model=QueryResponse)
async def ask_question(request: QueryRequest):
    if not rag_app: return {"answer": "Error: RAG agent not loaded."}
